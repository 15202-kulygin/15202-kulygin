...
void euler (graph ** vertexes, int size, int vertexquantity)
{
	int i = 0;
	int isfirst = 0;
	graph * start = NULL;	
	for (i = 0; i < size; ++i)
	{
		...
		while (NULL != vertexes[i])
		{
			...
			if ((vertexes[i]->ct == 0) || (vertexes[i]->ct % 2 == 1))
			{
				printf("There is no euler cycle\n");
				vertexes[i] = temp;
				return;
			}
			vertexes[i] = vertexes[i]->next;
	...
	STACK * last = (STACK *) calloc(1, sizeof(STACK));
	last->data = start;
	last->next = NULL;
	printf("Euler cycle : ");
	while (NULL != last)
	{
		graph * current = last->data;
		//printf("ID %d WITH ARR %d\n", current->id, current->ct);
		if (0 == current->ct)
		{
			printf("%d ", current->id);
			STACK * temp = last;
		...
		{
			graph * neighbor = current->arr[current->ct - 1];
			current->arr[current->ct - 1] = NULL;
			int currentindexinarr = searchneighbor(neighbor->arr, current->id, neighbor->ct);
			int k = 0;
			//printf("CIA %d\n", currentindexinarr);
			neighbor->arr[currentindexinarr] = NULL;
			for (k = currentindexinarr; k < neighbor->ct - 1; ++k)
			{
					neighbor->arr[k] = neighbor->arr[k+1];
			}
			neighbor->arr[k] = NULL;
			neighbor->ct--;
			current->ct--;
			STACK * newlast = (STACK *) calloc (1, sizeof(STACK));
			newlast->data = neighbor;
			newlast->next = last;
			last = newlast;
		}
	}
