...
int hash (char * key, long long int size) /* хэш-функция */
{
	long long int h = 0;
	int p = 1;
	int i = 0;
...
int search (char * key, ht ** hashtable, long long int size) /* поиск записи по ключу */
{
	int h = hash(key, size);
	ht * start = hashtable[h];
	while (NULL != hashtable[h])
	{
		...
		{
			printf("%s : %.1fsm - height, %.2fkg - weight\n", hashtable[h]->name, hashtable[h]->height, hashtable[h]->weight);
			return 1;
		}
		hashtable[h] = hashtable[h]->next;
	}
	hashtable[h] = start;
	printf ("The studen't wasn't found\n");
	return 0;	
}
