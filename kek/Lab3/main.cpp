...
int main(int argc, char **argv)
{
	...
	WriteToFileHandler handleWrite(0);
	...
	QObject::connect(&menu,SIGNAL(WriteToFile()),
					 &handleWrite,SLOT(Write()));
	QObject::connect(&handleWrite,SIGNAL(SaveIntoFile(std::string)),
					&f,SLOT(SaveToFile(std::string)));
