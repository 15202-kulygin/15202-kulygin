...
float* MatrixMult(float* first_matrix,float* second_matrix){
  ...
  float *transp=Matrix_Transporation(second_matrix);
  __m128 *m2 = (__m128*)transp;
  float *result = new float[N*N]();
  for(unsigned i = 0; i < N; ++i) {
    for(unsigned j = 0; j < N; ++j) {
      result[i*N+j]=inner2(first_matrix+i*N,transp+j*N);
    }
  }
  return result;
}
...
int main(){
  ...
  for (size_t i = 0; i < N; i++) {
    for (size_t j = 0; j < N; j++) {
      if (((i==j) && std::abs(1-matrix_C[i*N+j])>0.1) || ((i!=j) && (std::abs(matrix_C[i*N+j])>0.1))){
        std::cout<<"UNCORRECT";
        return 0;
      }
    }
